...
assert.match(app, /const RELEASE_VERSION = "5\.3\.3";/, "L’écran de nouveautés doit suivre la version livrée");
...
assert.match(html, /Mise à jour 5\.3\.3[\s\S]*?Quoi de neuf/, "L’écran de nouveautés doit annoncer clairement la version");
...
assert.match(html, /<strong>PDF net<\/strong>[\s\S]*?<strong>Joint auto<\/strong>[\s\S]*?<strong>À joindre<\/strong>/, "Les nouveautés doivent annoncer le PDF corrigé et les groupes d’envoi de la version 5.3.3");
assert.match(html, /<symbol id="icon-pdf"[^>]*>[\s\S]*?M12 10v7[\s\S]*?<use href="#icon-pdf">/, "Le téléchargement doit utiliser un pictogramme document avec flèche");
