...
assert.match(app, /const RELEASE_VERSION = "5\.3\.4";/, "L’écran de nouveautés doit suivre la version livrée");
...
assert.match(html, /Mise à jour 5\.3\.4[\s\S]*?Quoi de neuf/, "L’écran de nouveautés doit annoncer clairement la version");
...
assert.match(html, /<strong>Même ligne<\/strong>[\s\S]*?<strong>Sans gêne<\/strong>[\s\S]*?<strong>Survol<\/strong>/, "Les nouveautés doivent annoncer l’en-tête aligné de la version 5.3.4");
