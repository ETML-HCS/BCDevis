...
async function run() {
  ...
  try {
    ...
    const initial = await window.webContents.executeJavaScript(`(async () => {
      ...
      if (!releaseLayer || releaseLayer.hidden) throw new Error("L’écran des nouveautés 7.1.1 ne s’ouvre pas au premier lancement");
      if (localStorage.getItem("bcdevis-release-notes-last-seen") !== "7.1.1") throw new Error("La version des nouveautés n’est pas mémorisée");
