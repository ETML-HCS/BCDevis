  ...
  try {
    ...
    for (const viewport of [
      ...
      await window.webContents.executeJavaScript(`(() => {
        ...
        const toastSlot = document.querySelector("#checkoutToastSlot");
        const dateRect = document.querySelector(".receipt-meta label").getBoundingClientRect();
        const toastRect = toastRegion.getBoundingClientRect();
        if (toastRegion.parentElement !== toastSlot) throw new Error("La notification ne rejoint pas la date du devis à ${viewport.width}px");
        if (toastRect.left < dateRect.right + 4 || Math.abs(toastRect.bottom - dateRect.bottom) > 3) throw new Error("La notification n’est pas correctement alignée avec la date à ${viewport.width}px");
        document.querySelector("#moreQuoteButton").click();
        ...
        if (toastRegion.getBoundingClientRect().top < topbar.getBoundingClientRect().bottom) throw new Error("La notification de repli recouvre l’en-tête à ${viewport.width}px");
