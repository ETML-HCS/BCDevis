...
assert.equal(packageJson.version, "8.0.0", "La livraison doit annoncer la version 8.0.0");
...
assert.match(serviceWorker, /bcdevis-pwa-v8\.0\.0-touch-ipad-smartphone-documents-help/, "Le cache PWA doit suivre la version tactile, les réglages documentaires et le centre d’aide");
...
for (const document of [readme, manual, quick, shortcuts, templateGuide, clientReadme]) {
  assert.match(document, /8\.0\.0/, "Chaque document client doit annoncer la version 8.0.0");
}
...
assert.match(quick, /BCDevis-8\.0\.0\.exe/);
assert.match(quick, /BCDevis-8\.0\.0-linux-x86_64\.AppImage/);
assert.match(quick, /BCDevis-8\.0\.0-chromeos\.zip/);
assert.match(quick, /adresse HTTPS/);
assert.match(quick, /Réglages > Interface > Navigation/);
assert.match(shortcuts, /ChromeOS/);
assert.match(clientReadme, /3-ChromeOS[\s\S]*BCDevis-8\.0\.0-chromeos\.zip/);
