...
assert.equal(packageJson.version, "5.3.2", "La livraison doit annoncer la version 5.3.2");
...
assert.match(serviceWorker, /bcdevis-pwa-v5\.3\.2/, "Le cache PWA doit suivre la version livrée");
...
for (const document of [readme, manual, quick, shortcuts, templateGuide, clientReadme]) {
  assert.match(document, /5\.3\.2/, "Chaque document client doit annoncer la version 5.3.2");
}
...
assert.match(quick, /BCDevis-5\.3\.2\.exe/);
assert.match(quick, /BCDevis-5\.3\.2-linux-x86_64\.AppImage/);
assert.match(quick, /BCDevis-5\.3\.2-chromeos\.zip/);
assert.match(quick, /adresse HTTPS/);
assert.match(quick, /Réglages > Interface > Navigation/);
assert.match(shortcuts, /ChromeOS/);
assert.match(clientReadme, /3-ChromeOS[\s\S]*BCDevis-5\.3\.2-chromeos\.zip/);
