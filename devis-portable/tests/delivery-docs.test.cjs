...
assert.equal(packageJson.version, "7.1.3", "La livraison doit annoncer la version 7.1.3");
...
assert.match(serviceWorker, /bcdevis-pwa-v7\.1\.3-touch-ipad-smartphone-documents-help/, "Le cache PWA doit suivre la version tactile, les réglages documentaires et le centre d’aide");
...
for (const document of [readme, manual, quick, shortcuts, templateGuide, clientReadme]) {
  assert.match(document, /7\.1\.3/, "Chaque document client doit annoncer la version 7.1.3");
}
...
assert.match(quick, /BCDevis-7\.1\.3\.exe/);
assert.match(quick, /BCDevis-7\.1\.3-linux-x86_64\.AppImage/);
assert.match(quick, /BCDevis-7\.1\.3-chromeos\.zip/);
assert.match(quick, /adresse HTTPS/);
assert.match(quick, /Réglages > Interface > Navigation/);
assert.match(shortcuts, /ChromeOS/);
assert.match(clientReadme, /3-ChromeOS[\s\S]*BCDevis-7\.1\.3-chromeos\.zip/);
