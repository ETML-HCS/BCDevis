...
assert.equal(packageJson.version, "5.3.9", "La livraison doit annoncer la version 5.3.9");
...
assert.match(serviceWorker, /bcdevis-pwa-v5\.3\.9/, "Le cache PWA doit suivre la version livrée");
...
for (const document of [readme, manual, quick, shortcuts, templateGuide, clientReadme]) {
  assert.match(document, /5\.3\.9/, "Chaque document client doit annoncer la version 5.3.9");
}
...
assert.match(quick, /BCDevis-5\.3\.9\.exe/);
assert.match(quick, /BCDevis-5\.3\.9-linux-x86_64\.AppImage/);
assert.match(quick, /BCDevis-5\.3\.9-chromeos\.zip/);
assert.match(quick, /adresse HTTPS/);
assert.match(quick, /Réglages > Interface > Navigation/);
assert.match(shortcuts, /ChromeOS/);
assert.match(clientReadme, /3-ChromeOS[\s\S]*BCDevis-5\.3\.9-chromeos\.zip/);
