...
- Le **menu … du devis** s’ouvre désormais vers la droite et reste entièrement dans la caisse, au-dessus du panneau des soins.
- Le bouton **Catalogue** garde la même taille que **Réglages** et **Aide**, à toutes les largeurs.

## Nouveautés 7.1.6

- Le bouton **Catalogue** a la même taille que **Réglages** et **Aide** : la barre du haut est harmonisée.
- Le **menu … du devis** s’affiche toujours au-dessus du panneau des soins (correctif d’affichage).

## Nouveautés 7.1.5

- La **caisse** est plus lisible : état, nombre de soins et date sont réunis sur une seule ligne, et les boutons du haut sont alignés à gauche.
- Le raccourci **Ctrl+L** bascule la langue du **PDF** (FR / EN) en un instant, sans passer par les réglages ni le menu du devis.
- Un devis **enregistré** (Historique) réimporté revient dans le centre avec son statut et sa date.
- Le **menu d’actions** du devis est toujours visible (correction d’affichage) et le bouton **Aide** s’affiche en icône seule.

## Nouveautés 7.1.4

- Le **PDF en anglais** traduit aussi les **noms des soins** et présente les montants au format anglais.
- La **caisse** est plus compacte : son padding a été légèrement réduit.

## Nouveautés 7.1.3

- Le menu `...` du devis propose **PDF : FR / EN** : un clic bascule la langue du PDF sans passer par les réglages.

## Nouveautés 7.1.2

- **Réglages > Devis & suivi** permet de générer le PDF du devis en **anglais**, sans changer l’interface de BCDevis.
- L’import des contacts **vCard** lit correctement les retours à la ligne et les caractères échappés.
- La **synchronisation centrale** valide chaque échange dans une transaction et ne laisse aucun état partiel.
...
- Distribuer uniquement `BCDevis-7.1.7.exe`, généré dans `dist`. C’est l’unique fichier à lancer : aucun navigateur ni installation ne sont nécessaires.
...
Le livrable est `devis-portable/dist/chromeos/BCDevis-7.1.7-chromeos.zip`. Il contient le dossier statique `site` à publier sur un hébergement HTTPS et une notice d’installation.
...
Le fichier à remettre est `devis-portable/dist/BCDevis-7.1.7.exe`. Ne pas distribuer le dossier `win-unpacked`, qui ne sert qu’à la fabrication.
...
Le fichier à distribuer est `devis-portable/dist/BCDevis-7.1.7-mac.dmg`. Cette commande doit être exécutée depuis un Mac ou un runner CI macOS ; elle est volontairement bloquée sous Windows et Linux. Une signature et une notarisation Apple sont nécessaires avant une diffusion large pour éviter les alertes Gatekeeper.
...
Le fichier à distribuer est `devis-portable/dist/BCDevis-7.1.7-linux-x86_64.AppImage`. Cette commande doit être exécutée depuis Linux ou un runner CI Linux ; elle est volontairement bloquée sous Windows et macOS. Une fois généré, le fichier doit être rendu exécutable avec `chmod +x` avant son premier lancement.
